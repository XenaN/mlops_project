    ...
    with open(f"{save_path}urls.txt", "wb") as urls_file:
        for country in countries:
            for pollutant in pollutants:
                download_file = (
                    f"https://fme.discomap.eea.europa.eu/"
                    f"fmedatastreaming/AirQualityDownload/"
                    f"AQData_Extract.fmw?CountryCode="
                    f"{country}&CityName=&Pollutant="
                    f"{metadata[pollutant]}&Year_from="
                    f"{year_start}&Year_to={year_end}&"
                    f"Station=&Samplingpoint=&Source=All&Output="
                    f"TEXT&UpdateDate=&TimeCoverage=Year"
                )
                try:
...
def save_csv_from_url(pair_path_and_url: Tuple):
    ...
    try:
        ...
        dataset.to_csv(
            f"{pair_path_and_url[0]}" f"{pair_path_and_url[1].split('/')[-1]}",
            index=False,
        )
    except Exception:
...
def download_historical_data_from_discomap_urls(
    ...
    download_urls_historical_data_from_discomap(
        save_path, metadata_path, countries, pollutants, year_start, year_end
    )
...
def download_updated_data_from_discomap(
    save_path: str, url: str, countries: List, pollutants: List
):
    """
