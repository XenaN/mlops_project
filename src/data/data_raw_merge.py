...
def merge_eea_data(country: str, pollutant: str, metadata_path: str):
    """
    ...
    if len(os.listdir(INTERIM_UPDATED_EEA_PATH)) == 0:
        ...
        pathlib.Path(f"{INTERIM_UPDATED_EEA_PATH}{country}_{pollutant}/").mkdir(parents=True, exist_ok=True)
    else:
        path = f"{INTERIM_UPDATED_EEA_PATH}{country}_{pollutant}/"
    ...
    for file in list_csv:
        ...
        data_new.to_csv(f"{INTERIM_UPDATED_EEA_PATH}{country}_{pollutant}/{file}", index=False)
